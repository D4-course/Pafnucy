...
def input_dir(path):
    ...
    for dataset_name in datasets:
        dataset_path = os.path.join(path, f'{dataset_name}_set.hdf' )
        if not os.path.exists(dataset_path):
            raise IOError(f'Incorrect input_dir specified:'
                          ' {dataset_path} set file not found')
    return path
...
for dataset_name in datasets:
    dataset_path = os.path.join(args.input_dir, f'{dataset_name}_set.hdf')
...
def get_batch(dataset_name, indices, rotation=0):
    ...
    for _, idx in enumerate(indices):
        ...
        x_lis.append(make_grid(coords_idx, features_idx,
                 grid_resolution=args.grid_spacing,
                 max_dist=args.max_dist))
    x_lis = np.vstack(x_lis)
    x_lis[..., columns['partialcharge']] /= std
    return x_lis
...
print(f'baseline mse: training={t_baseline}, validation={v_baseline}')
...
for set_name, set_size in ds_sizes.items():
    print(f'{set_size} {set_name} samples')
...
print('')
print(f'convolutional layers: {", ".join((str(i) for i in args.conv_channels))}'
            f'channels, {args.conv_patch}A patch + max pooling with {args.pool_patch}A patch')
print('fully connected layers:', ', '.join((str(i) for i in args.dense_sizes)),
      'neurons')
print(f'regularization: dropout (keep {args.kp}) and L2 (lambda {args.lmbda})')
print('')
...
with tf.Session(graph=graph) as session:
    ...
    summary_imp.value.add(tag=f'feature_importance_{0}' , image=image)
    ...
    for epoch in range(args.num_epochs):
        ...
        # SAVE MODEL #
        print(f'epoch: {epoch} train error: {mse_t}, validation error: {mse_v}')
